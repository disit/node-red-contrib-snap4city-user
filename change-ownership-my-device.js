...
module.exports = function (RED) {
    ...
    function ChangeOwnershipMyDevice(config) {
        ...
        node.on('input', function (msg) {
			...
			var newownership = (msg.payload.newownership ? msg.payload.newownership : config.newownership);
            ...
            try {
			  ...
			  node.error("Contextbroker not configured or not sent to input");
			  return;
			}
            ...
            if (elementName || contextbroker!="") {
                ...
                if (accessToken != "" && typeof accessToken != "undefined") {
					if (contextbroker != "" && typeof contextbroker != "undefined") {
						...
						logger.info(encodeURI(uri + "?action=change_owner&newowner="+newownership+"&id=" + elementName + "&contextbroker=" + contextbroker +"&k1=&k2=&token=" + accessToken + "&nodered=yes"));
						xmlHttp.open("POST", encodeURI(uri + "?action=change_owner&newOwner="+newownership+"&id=" + elementName + "&contextbroker=" + contextbroker +"&k1=&k2=&model=&token=" + accessToken + "&nodered=yes"), true);
