...
//##
//# versione di Isidori F. - pensare alla licenza prima di github
//#
...
wst_client_reverse1 = function() {
  this.wsClientForControll = new WebSocketClient();
};

//wst_client_reverse1.prototype.start = function(portTunnel, wsHostUrl, remoteAddr) {
  wst_client_reverse1.prototype.start = function(idClient, wsHostUrl, remoteAddr) {
  ...
  url = "" + wsHostUrl + "/?dst=" + urlWsHostObj.hostname+":"+idClient;
  
  logger.info("Versione modificata Isidori F.");
  logger.info("[SYSTEM] - Connecting to", wsHostUrl);
  //logger.info("[SYSTEM] --> exposing", remoteAddr, "on port", portTunnel);
  logger.info("[SYSTEM] --> exposing", remoteAddr, "id client", idClient);
...
module.exports = wst_client_reverse1;
