...
//##
//#versione con modifiche Isidori F. - occuparsi delle licenze prima di rilasciare qualsiasi cosa
//#
...
// importo mia funzione per accesso al DB MySql
var accessoDb = require("./accessoDb.js");

logger.info("WSTUN STARTED! Versione modificata Isidori F.");
...
// Isidori F. portTcp contiene la porta di destinazione indicata dal client TODO devo modificare in modo di scegliere in altro modo
       // var portTcp = ref1[1];
        var idClient = ref1[1];
	//var portTcp = 8081; // per ora 8081 fissa
	      
        //logger.info("[SYSTEM] WebSocket creation towards " + src_address + " on port " + portTcp );
        logger.info("[SYSTEM] WebSocket creation call towards " + src_address + " for client " + idClient );
     
        // utilizzando accessodb cerco se il cliente e' noto, in tal caso mi viene assegnata una porta
	accessoDb.cercaClient(idClient).then(function(portTcp) {
          
          logger.info("[SYSTEM] DB assigned port for client " + portTcp );
  	  ...
  	  /* todo gestire errore - puo' essere problema db o client sconosciuto */
	  logger.info("[SYSTEM] --> Accesso DB fallito: "+reason);
	  // tentare di rilanciare una ecc di tipo 'error' di net.events
	});
