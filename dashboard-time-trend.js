...
module.exports = function (RED) {
    ...
    function DashboardTimeTrendNode(config) {
        ...
        node.on('input', function (msg) {
            ...
            setTimeout(function () {
                try {
                    ...
                    util.log("Error sending data to WebSocket for dashboard-time-trend node " + node.name + ": " + JSON.stringify(e));
                }
            }, timeout);
        ...
        node.wsOpenCallback = function () {
            if (node.dashboardTitle != null && node.dashboardTitle != "") {
                ...
                util.log("time-trend node " + node.name + " IS GOING TO CONNECT TO WS");
    ...
    RED.httpAdmin.get("/retrieveAccessTokenLocal/", RED.auth.needsPermission('dashboard-time-trend.read'), function (req, res) {
        var s4cUtility = require("./snap4city-utility.js");
        res.json({
