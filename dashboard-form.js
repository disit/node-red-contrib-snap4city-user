...
module.exports = function (RED) {
    ...
    function DashboardForm(config) {
        ...
        node.dropdwonOrderedList = config.dashboardFormOrderedList;
        node.httpRoot = null;
		node.selected = null;
        ...
        node.on('input', function (msg) {
            ...
            if (typeof node.widgetUniqueName != "undefined" && node.widgetUniqueName != "") {
				...
				if (typeof msg.payload.form != "undefined"){
                    node.dashboardFormOrderedList = msg.payload.form;
                }
                var newMetricData = {
                    msgType: "SendToEmitter",
                    widgetUniqueName: node.widgetUniqueName,
                    dashboardId: node.dashboardId,
                    value: JSON.stringify(node.dashboardFormOrderedList)
                }; 
                ...
                node.error("Maybe the dashboardForm is not created on dashboard");
                logger.error("Error, the dashboardForm is not created on dashboard. node.widgetUniqueName: " + node.widgetUniqueName);
            }
        });
        ...
        node.wsOpenCallback = function () {
            if (node.dashboardId != null && node.dashboardId != "") {
                ...
                var payload = {
                    ...
                    user: node.username,
                    startValue: JSON.stringify({"form":[]}),
		...
		node.deliverInitialConfig = function() {
            ...
            if (typeof node.widgetUniqueName != "undefined" && node.widgetUniqueName != "") {
                ...
                /*var currentdashboardFormOrderedList = [];

                for (var i = 0; i < node.dropdwonOrderedList.length; i++) {
                    if (node.dropdwonOrderedList[i].value != "") {
                        currentdashboardFormOrderedList.push([node.dropdwonOrderedList[i].label, node.dropdwonOrderedList[i].value]);
                    }
                }
				
				if(currentdashboardFormOrderedList.length > 0) {*/
    ...
    RED.nodes.registerType("dashboardForm", DashboardForm);
