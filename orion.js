...
module.exports = function (RED) {
	...
	function OrionService(config) {
		...
		if (/\/$/.test(this.url)) {
			this.url = this.url.substring(0,this.url.length - 1);//remove the final / if exist
		}
		...
		this.queryContext = function (node, config, payload) {
			...
			return when.promise(function (resolve, reject) {
				...
				getContextBrokerListForRegisterActivity(node, orionBrokerService.url, orionBrokerService.port, retrieveDeviceName(node, config.enid, config.tenant, config.servicepath), uid, accessToken);
							
				var urlWithoutHttp = orionBrokerService.url;			
		...
		this.updateContext = function (node, config, payload, auth) {
			...
			return when.promise(function (resolve, reject) {
				...
				getContextBrokerListForRegisterActivity(node, orionBrokerService.url, orionBrokerService.port, retrieveDeviceName(node, payload.contextElements[0].id, config.tenant, config.servicepath), uid, accessToken);
		...
		if (typeof node.deviceLongId != "undefined") {
			...
			if (accessToken != "" && typeof accessToken != "undefined") {
				...
				xmlHttp.onload = function (e) {
					if (xmlHttp.readyState === 4) {
						if (xmlHttp.status !== 200) {
							logger.error("Error retrieved from lightActivities");
							logger.error(xmlHttp.statusText);
						}
					} else {
						logger.error(xmlHttp.statusText);
