...
module.exports = function (RED) {
    ...
    function SpeakSynthesisNode(config) {
        ...
        node.on('input', function (msg) {
			...
			/////////////////////Modifiche
			var text = (msg.payload.text ? msg.payload.text : config.text);
			var rate = (msg.payload.rate? msg.payload.rate : config.rate);	
			var pitch = (msg.payload.pitch ? msg.payload.pitch : config.pitch);
			...
			var valueWidget={"text":text,"pitch_value":pitch,"rate_value":rate,"lang":lang};
            /////////////////////Modifiche
			...
			var newMetricData = {
                ...
                newValue: JSON.stringify(valueWidget),////Cambiato prima era msg.payload
                appId: uid,
                user: node.username,
                flowId: node.z,
            ...
            setTimeout(function () {
                try {
					...
					logger.info("Send AddMetricData to WebSocket: " + JSON.stringify(config));
        ...
        node.wsOpenCallback = function () {
            if (node.dashboardId != null && node.dashboardId != "") {
                ...
                var payload = {
                    ...
                    flowId: node.z,
                    flowName: node.flowName,
                    widgetType: "widgetSpeakSynthesis",
...
node.wsCloseCallback = function (e) {
    ...
    RED.nodes.registerType("speak-synthesis", SpeakSynthesisNode);
